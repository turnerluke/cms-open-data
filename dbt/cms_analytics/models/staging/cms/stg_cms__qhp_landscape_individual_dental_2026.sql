with source as (
    ...
    select * from {{ source('cms_raw', 'cms_qhp_landscape_individual_dental_2026') }}
...
renamed as (
    ...
    select
        ...
        -- identifiers (grain: plan_id × fips_county_code). Stand-
        -- alone dental plans use two metal levels (`High`, `Low`),
        -- not the medical bronze/silver/gold/platinum ladder.
        ...
        -- coverage indicators — CMS ships eight `(Coverage)` flag
        -- columns telling you which dental services are covered
        -- (values like `Covered`, `Not Covered`, `Not Applicable`).
        ...
        -- premium scenarios (37 age/composition cells). Note: dental
        -- files spell 3-child brackets as `Couple+3 children` /
        -- `Individual+3 children` (no `or more`); medical files use
        -- `Couple+3 or more Children`. We normalise the column
        -- suffix to `_3_or_more_children_` so unions across dental
        -- and medical align.
        ...
        -- Standard dental cost-sharing block. Deductibles and MOOPs
        -- cast to decimal; copay/coinsurance text (`$50 Copay after
        -- deductible`, `Not Applicable`, `No Charge`) stays as text.
        -- There is no drug block on dental plans.
        ...
        -- Text companion for the family dental deductible. The
        -- numeric cast above silently drops ~56% of rows to the
        -- sentinels `See Plan Brochure` (~44%) and `Not Applicable`
        -- (~12%); we keep the raw trimmed string so downstream
        -- consumers can distinguish those cases from a genuine
        -- NULL.
        nullif(trim("Dental Deductible - Family - Standard"), '') as dental_deductible_family_standard_raw,
