...
#
# Replaces a hand-written one-liner orchestration sessions used to launch
# in the background to babysit a PR. Two things the one-liner got wrong
# and this script fixes:
#
#   1. it printed "ALL_PASS" unconditionally, even when a check had
#      failed — a reader had to notice fail lines above the marker.
#      Here the final marker is `CI_PASS` XOR `CI_FAIL` and reflects the
#      true state.
#   2. if a failed check was fixed and CI re-ran, the one-liner had
#      already left its CI phase and never re-checked. Here `CI_FAIL`
#      loops back into the pending-wait phase so a fix-push is
#      naturally picked up.
#
...
# Polling intervals (seconds). Tuned to match the one-liner they replace.
readonly CI_POLL_INTERVAL=20
readonly MERGE_POLL_INTERVAL=30
...
# Using awk on the status column avoids the one-liner's substring-grep
# bug (a check named `bypass-something` used to match `-v pass`).
...
while :; do
    ...
    # picked up. Suppressing identical repeats keeps the orchestrator's
    # tailed log from ballooning while a human works on the fix.
