...
def test_bulk_csv_asset_written_for_every_registry_row() -> None:
    """Every `dkan_data_api_bulk` spec should bind a module attribute."""
    bulk_specs = [s for s in load_registry() if s.source == "dkan_data_api_bulk"]
    assert bulk_specs, "expected at least one dkan_data_api_bulk row"
